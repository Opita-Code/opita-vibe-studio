...
- [x] 9.2 TipBanner.tsx + KnowledgeGarden.tsx: "¿Sabías que...?" banner and knowledge garden (seedling/sprout/tree icons)
- [x] 9.3 Event triggers: scanAndTrigger wired into ChatPanel (after pipeline/direct chat) and EditorPanel (on save)
- [x] 9.4 TerminalPanel.tsx: command input, spinner, stdout/stderr output, exit code, 30s timeout display
- [x] 9.5 Spanish translation: terminal-translations.ts: file-not-found, perm-denied, cmd-not-found, git/npm status output
...
- [x] 10.6 tests/lib/language.test.ts: detectLanguage for all 15+ extensions and edge cases (19 tests)
- [x] 10.7 tests/lib/git.test.ts: getGitBranch, getGitStatus parsing, error handling (11 tests)
- [x] 10.8 tests/integration/chat-flow.test.ts: full chat flow, provider failover, BYOK, auth, token limits (18 tests)
- [x] 10.9 tests/integration/edge-cases.test.ts: empty project, large files, special chars, network failure, malformed JSON, context overflow (15 tests)
- [x] 10.10 tests/components/chat/a11y.test.tsx: tab navigation, focus management, keyboard, ARIA labels (7 tests)
- [x] 10.11 tests/stores/ui.test.ts: added togglePreview, toggleTerminal, setTerminalHeight (5 tests added)
- [x] 10.12 tests/stores/learning.test.ts: added queue management, duplicate prevention (2 tests added)
- [x] 10.13 tests/stores/auth.test.ts: added setUser, plan switch, tokenUsage edge cases (4 tests added)
- [x] 10.14 Coverage thresholds: 70% lines, 70% branches, 60% functions added to vite.config.ts

## Phase 11: Polish + Security

- [x] 11.1 Security audit: CSP (connect-src 'none' in srcdoc), sandbox (allow-scripts only, verified by test), console.log audit (no API key leakage), BYOK masking (maskKey()), no eval/innerHTML in user content, Tauri capabilities notes
- [x] 11.2 Live preview security hardening: integration tests for iframe sandbox isolation, CSP meta tag verification, connect-src 'none' enforcement, srcdoc vs innerHTML/eval checks (12 tests)
- [x] 11.3 UI polish: ErrorBoundary component wrapping all panels, empty state for chat (no messages), transition animations (panel resize, chat toggle, terminal show/hide, TipBanner fade)
- [x] 11.4 Quality gates: husky pre-commit hook with lint-staged (format + lint on commit), npm run health script (typecheck + lint + format:check + test), lint-staged config for ts/tsx/json/md/css
- [x] 11.5 Spec compliance: cross-referenced all 49 scenarios from 12 specs — all covered by existing tests OR explicitly noted as documented future capability (file watching, E2E loop, boot perf are deferred to post-MVP)

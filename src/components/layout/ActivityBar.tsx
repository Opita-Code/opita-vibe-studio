...
export function ActivityBar({ onLogin }: { onLogin?: () => void }) {
  ...
  return (
    <div className="w-12 h-full bg-obsidian-950 border-r border-white/5 flex flex-col items-center py-3 flex-shrink-0 z-40 select-none">
      {/* Top: Logo / Branding */}
      ...
      <div className="flex flex-col gap-4 flex-1 w-full items-center mt-2">
        {/* Explorer */}
        <button
        ...
        >
          {activeSidebar === "explorer" && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 bg-aura-cyan shadow-[0_0_8px_rgba(6,182,212,0.6)]"></div>
          )}
        ...
        >
          {activeSidebar === "chat" && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 bg-aura-purple shadow-[0_0_8px_rgba(168,85,247,0.6)]"></div>
          )}
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="flex flex-col gap-4 w-full items-center">
        ...
        >
          {settingsVisible && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-8 bg-white/50"></div>
          )}
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* User Account */}
        <div className="w-full flex justify-center py-2 relative group mb-2">
          {authMode === "unauthenticated" ? (
            <button
              onClick={onLogin}
            ...
            <button
              onClick={logout}
              className="w-8 h-8 rounded-full bg-aura-purple/20 border border-aura-purple/30 text-aura-purple hover:bg-red-500/20 hover:border-red-500/30 hover:text-red-400 flex items-center justify-center transition-all"
              title={`${user?.email || "Usuario"} (Click para cerrar sesión)`}
            >
              <span className="text-[12px] font-bold group-hover:hidden">
                {user?.email ? user.email.charAt(0).toUpperCase() : "U"}
              </span>
              <svg className="w-4 h-4 hidden group-hover:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">

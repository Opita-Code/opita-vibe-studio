...
export function createMistralProvider(apiKey?: string, model?: string): AIProvider {
  const key = apiKey ?? "";
  ...
  const provider: AIProvider = {
    ...
    chat: async function* (messages: Message[], options?: ChatOptions): AsyncGenerator<ChatChunk> {
      if (!configured) {
        yield { type: "error", content: "Mistral no configurado." };
        return;
      }
      ...
      try {
        ...
        if (err instanceof SseError) {
          yield { type: "error", content: `Mistral error (${err.status}): ${err.message}` };
        } else {
          yield { type: "error", content: `Mistral error inesperado: ${String(err)}` };
        }
      }
    },
    countTokens,

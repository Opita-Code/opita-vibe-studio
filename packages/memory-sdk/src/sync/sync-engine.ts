/**
 * SyncEngine — orchestrates bidirectional sync between local storage and AWS cloud.
 *
 ...
 */
import type { StorageBackend, LearningEvent } from "../types";
import { ContextDecayEngine } from "./context-decay";
...
export interface SyncEngineOptions {
  ...
  decayEngine?: ContextDecayEngine;
}

export class SyncEngine {
  private storage: StorageBackend;
  private cloudBridge: CloudBridgeClient;
  private decayEngine?: ContextDecayEngine;

  constructor(options: SyncEngineOptions) {
    this.storage = options.storage;
    this.cloudBridge = options.cloudBridge;
    this.decayEngine = options.decayEngine;
  }
  ...
  async pull(userId: string): Promise<void> {
    ...
    for (const key of cloudKeys) {
      ...
      if (!localEntry || cloudEntry.timestamp >= localEntry.timestamp) {
        let value = cloudEntry.value;
        let timestamp = cloudEntry.timestamp;

        if (key === "events" && Array.isArray(value) && this.decayEngine) {
          value = this.decayEngine.decay(value as LearningEvent[]);
          timestamp = Date.now(); // update timestamp so it gets pushed back to the cloud on push
        }
